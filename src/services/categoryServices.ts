...
export const getAllCategories = async (): Promise<ICategory[]> => Category.find().sort({ name: 1 }).lean().exec();
...
export const updateCategory = async (id: string, name: string): Promise<ICategory | null> =>
  Category.findByIdAndUpdate(id, { name }, { new: true, runValidators: true }).lean().exec();

export const getCategoryById = async (id: string): Promise<ICategory | null> => Category.findById(id).lean().exec();
